...
def descobrir_pagina_reposicoes(page):
    # Primeiro tenta links/botões que explicitamente mencionem reposição/agendamento.
...
def extrair_reposicoes(page, unidade):
    ...
    for ti in range(tabelas.count()):
        ...
        for ri in range(linhas.count()):
            row = linhas.nth(ri)
            celulas = [texto(row.locator("td").nth(ci)) for ci in range(row.locator("td").count())]
            bruto = " | ".join(celulas)
            if not bruto or len(celulas) < 2:
                continue
            data = normalizar_data(bruto)
            if not re.search(r"\d{1,2}/\d{1,2}/\d{2,4}|\d{4}-\d{2}-\d{2}", bruto):
                continue
            inicio = normalizar_horario(bruto)
            ...
            nome = ""
            for c in celulas:
                if not contrato and re.fullmatch(r"\d{4,}", c.replace(".", "").replace("-", "")):
                    contrato = c
                if not nome and len(c.split()) >= 2 and not re.search(r"\d{1,2}/\d{1,2}/\d{2,4}", c):
                    if not any(k in c.lower() for k in ["agendada", "realizada", "cancelada", "reposição", "reposicao"]):
                        nome = c
            if not nome:
                nome = celulas[0]
            ...
            for c in celulas:
                lc = c.lower()
                if any(k in lc for k in ["informática", "informatica", "módulo", "modulo", "disciplina"]):
                    disciplina = c
                    break
    ...
    with sync_playwright() as p:
        ...
        for unidade in UNIDADES:
            ...
            try:
                ...
                print(f"[{unidade['nome']}] {len(reposicoes)} reposições sincronizadas")
